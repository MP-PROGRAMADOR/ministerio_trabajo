...
try {
    ...
    ni.fecha_creacion,
    ni.motivo_empresa,
    ni.buscador_id,                   -- 👈 Agregamos el id del buscador
    
    -- ID y campos de la tabla postulaciones
    p.id AS postulacion_id,
    p.estado AS estado_postulacion,
    p.mensaje_presentacion,
    p.fecha_postulacion,

    -- Datos del buscador de empleo
...
LEFT JOIN postulaciones p 
       ON p.oferta_id = ni.oferta_id 
      AND p.buscador_id = ni.buscador_id
                ...
                <div class="custom-card">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h5 class="fw-bold m-0">
                            ...
                            <span class="badge bg-light text-dark border ms-2"><?php echo count($intermediaciones); ?>
                                total</span>
                        </h5>
                                        ...
                                        <tr>
                                            <td>
                                                <span class="font-monospace fw-bold text-primary">
                                                    <?php echo htmlspecialchars($inter['notificacion_id']); ?>
                                                </span>
                                            </td>
                                            <td>
                                                <span class="font-monospace fw-bold text-primary">
...
<script>
    ...
    if (modalDetalle) {
        modalDetalle.addEventListener('show.bs.modal', function (event) {
            ...
            document.getElementById('modal-provincia').textContent = button.dataset.provincia || 'N/A';
            document.getElementById('modal-empresa').textContent   = button.dataset.empresa || 'N/A';
            document.getElementById('modal-puesto').textContent    = button.dataset.puesto || 'N/A';
            document.getElementById('modal-ruc').textContent       = button.dataset.ruc || 'N/A';
